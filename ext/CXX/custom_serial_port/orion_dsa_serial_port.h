 ...
 * Orion boards.
 */
class OrionDSASerialPort : public sp::IfSerialPort
{
    OrionDSASerialPort() = delete;
    OrionDSASerialPort(const OrionDSASerialPort&) = delete;
public:
    OrionDSASerialPort(unsigned int board_id, const std::string& port_name)
        : m_board_id(board_id)
    ...
    virtual BOOL SP_IMPORT read(BYTE* buf, std::size_t buf_size, unsigned int timeout, sp::ReadResult* read_result) override
    {
        ...
        if (bytes_available <= 0)
        {
            ...
            unsigned int delta_ms = static_cast<unsigned int>((time_us - start_time_us) / 1000);
            if (delta_ms < timeout)
            {
                auto wait_ms = timeout - delta_ms;
